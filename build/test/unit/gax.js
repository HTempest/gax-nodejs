...
function expectRetryOptions(obj) {
    chai_1.expect(obj).to.be.an.instanceOf(Object);
    chai_1.expect(obj).to.have.all.keys('retryCodes', 'backoffSettings');
    chai_1.expect(obj.retryCodes).to.be.an.instanceOf(Array);
    expectBackoffSettings(obj.backoffSettings);
}
function expectBackoffSettings(obj) {
    chai_1.expect(obj).to.be.an.instanceOf(Object);
    chai_1.expect(obj).to.have.all.keys('initialRetryDelayMillis', 'retryDelayMultiplier', 'maxRetryDelayMillis', 'initialRpcTimeoutMillis', 'rpcTimeoutMultiplier', 'maxRpcTimeoutMillis', 'totalTimeoutMillis');
}
describe('gax construct settings', () => {
    it('creates settings', () => {
        const otherArgs = { key: 'value' };
        const defaults = gax.constructSettings(SERVICE_NAME, A_CONFIG, {}, RETRY_DICT, otherArgs);
        let settings = defaults.bundlingMethod;
        chai_1.expect(settings.timeout).to.eq(40000);
        expectRetryOptions(settings.retry);
        chai_1.expect(settings.retry.retryCodes).eql([1, 2]);
        chai_1.expect(settings.otherArgs).eql(otherArgs);
        settings = defaults.pageStreamingMethod;
        chai_1.expect(settings.timeout).to.eq(30000);
        expectRetryOptions(settings.retry);
        chai_1.expect(settings.retry.retryCodes).eql([3]);
        chai_1.expect(settings.otherArgs).eql(otherArgs);
    });
    it('overrides settings', () => {
        ...
        const defaults = gax.constructSettings(SERVICE_NAME, A_CONFIG, overrides, RETRY_DICT);
        let settings = defaults.bundlingMethod;
        chai_1.expect(settings.timeout).to.eq(40000);
        settings = defaults.pageStreamingMethod;
        chai_1.expect(settings.timeout).to.eq(30000);
        chai_1.expect(settings.retry).to.eq(null);
    });
    it('overrides settings more precisely', () => {
        ...
        chai_1.expect(backoff.initialRetryDelayMillis).to.eq(1000);
        chai_1.expect(settings.retry.retryCodes).to.eql([RETRY_DICT.code_a]);
        chai_1.expect(settings.timeout).to.eq(50000);
        ...
        chai_1.expect(backoff.initialRetryDelayMillis).to.eq(100);
        chai_1.expect(backoff.retryDelayMultiplier).to.eq(1.2);
        chai_1.expect(backoff.maxRetryDelayMillis).to.eq(1000);
        chai_1.expect(settings.retry.retryCodes).to.eql([RETRY_DICT.code_c]);
    });
});
