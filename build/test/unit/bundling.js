...
const bundleDescriptor_1 = require("../../src/bundlingCalls/bundleDescriptor");
const bundleExecutor_1 = require("../../src/bundlingCalls/bundleExecutor");
const bundlingUtils_1 = require("../../src/bundlingCalls/bundlingUtils");
const task_1 = require("../../src/bundlingCalls/task");
...
describe('computeBundleId', () => {
    describe('computes the bundle identifier', () => {
        ...
        testCases.forEach(t => {
            it(t.message, () => {
                chai_1.expect(bundlingUtils_1.computeBundleId(t.object, t.fields)).to.equal(t.want);
            });
        });
    });
    describe('returns undefined if failed', () => {
        ...
        testCases.forEach(t => {
            it(t.message, () => {
                ...
                chai_1.expect(bundlingUtils_1.computeBundleId(t.object, t.fields)).to.be.undefined;
            });
        });
    });
});
describe('deepCopyForResponse', () => {
    it('copies deeply', () => {
        ...
        const output = task_1.deepCopyForResponse(input, null);
    ...
    it('respects subresponseInfo', () => {
        ...
        const output = task_1.deepCopyForResponse(input, {
            field: 'foo',
            start: 0,
            end: 2,
        });
        ...
        const output2 = task_1.deepCopyForResponse(input, {
            field: 'foo',
            start: 2,
            end: 4,
        });
...
describe('Task', () => {
    ...
    function testTask(apiCall) {
        return new task_1.Task(apiCall, {}, 'field1', null);
    }
    let id = 0;
        ...
        describe('increases the byte size', () => {
            ...
            testCases.forEach(t => {
                it(t.message, () => {
                    ...
                    extendElements(task, t.data);
                    chai_1.expect(task.getRequestByteSize()).to.eq(baseSize + t.want * sizePerData);
                });
            });
        });
    });
            ...
            testCases.slice(1).forEach(t => {
                it(t.message, done => {
                    ...
                    const callback = sinon.spy((e, data) => {
                        ...
                        // tslint:disable-next-line no-unused-expression
                        chai_1.expect(data).to.be.undefined;
                        if (callback.callCount === t.data.length) {
                            chai_1.expect(apiCall.callCount).to.eq(1);
                            done();
                        }
...
describe('Executor', () => {
    ...
    function newExecutor(options) {
        const descriptor = new bundleDescriptor_1.BundleDescriptor('field1', ['field2'], 'field1', byteLength);
        return new bundleExecutor_1.BundleExecutor(options, descriptor);
    }
